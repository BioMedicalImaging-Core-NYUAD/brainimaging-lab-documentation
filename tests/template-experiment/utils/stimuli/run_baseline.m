function [pa, exitFlag, currentFixationAngle, blinkCounter] = run_baseline(VP, pa, kb, experimentStartTime, currentFixationAngle, blinkCounter, blinkFrameThresh)
% RUN_BASELINE - Run baseline period with moving dot only
%
% Input:
...
baselineStartTime = GetSecs;
baselineEndTime = baselineStartTime + pa.baselineDuration;
vbl = baselineStartTime;
exitFlag = 0;

while GetSecs < baselineEndTime
    currentTime = GetSecs;
    ...
    if pressed && firstPress(kb.escKey)
        fprintf('\n*** Experiment terminated by user (ESC pressed during baseline) ***\n');
        exitFlag = 1;
        break;
    end
