...
def test_matlab_runs_from_env():
    """
    Verify MATLAB launches headlessly using the absolute path from env.
    Uses optional ARCH_* to force Apple Silicon execution when needed.
    """
    matlab_bin = os.environ.get("MATLAB_BIN") or os.environ.get("MATLAB_PATH")
    assert matlab_bin, "MATLAB_BIN or MATLAB_PATH must be set"
    ...
    cmd = prefix + [
        ...
        "-nodisplay", "-nosplash", "-nojvm",
        "-r",
        "try, disp('MATLAB command OK'); exit(0); catch ME, disp(getReport(ME,'extended')); exit(1); end",
    ]

    result = subprocess.run(cmd, capture_output=True, text=True, timeout=180)
    sys.stdout.write(result.stdout)
    sys.stderr.write(result.stderr)
    assert result.returncode == 0, f"MATLAB did not run successfully, rc={result.returncode}"
    assert "MATLAB command OK" in result.stdout, "Expected confirmation string not found in MATLAB output"
