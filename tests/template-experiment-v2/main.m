...
% Initialize eyetracking (using uncertainty experiment method)
if debugConfig.eyetracking
    dummymode = 0;  % Set to 1 for dummy mode (testing without hardware)
    [pa.EL, pa] = SetupEyetracking(VP, pa, dummymode);
    if isempty(pa.EL)
        fprintf('Eye tracking setup failed. Disabling eye tracking.\n');
...
try
    ...
    % Start recording (uncertainty method)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    % Stop and save Eyelink data (guarded) - using uncertainty method
    if pa.eyeTrackingEnabled
        try
            Eyelink('StopRecording');
            Eyelink('SetOfflineMode'); % Put tracker in idle/offline mode
            Eyelink('Command', 'clear_screen 0'); % Clear Host PC backdrop graphics
            WaitSecs(0.5); % Allow some time before closing and transferring file
            Eyelink('CloseFile'); % Close EDF file on Host PC

            % Transfer EDF file to Display PC
            edfFile = [pa.eyeFileBase '.edf'];
            fprintf('Receiving data file ''%s''\n', edfFile);
            status = Eyelink('ReceiveFile', pa.eyeFileBase, pa.eyeDataDir, 1);

            % Check if EDF file has been transferred successfully
            if status > 0
                fprintf('EDF file size: %.1f KB\n', status/1024);
                fprintf('Data file ''%s'' can be found in ''%s''\n', edfFile, pa.eyeDataDir);
            else
                fprintf('Problem receiving data file ''%s''\n', edfFile);
            end

            fprintf('  Eye tracking data saved and connection closed\n');
        catch ME
            fprintf('  Warning: Could not properly close eye tracking: %s\n', ME.message);
        end
    end
    ...
    % Save eye tracking data even if error occurred - using uncertainty method
    if exist('pa', 'var') && isfield(pa, 'eyeTrackingEnabled') && pa.eyeTrackingEnabled
        try
            Eyelink('StopRecording');
            ...
            fprintf('Warning: Could not save eye tracking data after error: %s\n', ME2.message);
        end
    end
end
