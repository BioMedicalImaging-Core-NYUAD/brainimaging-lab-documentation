% =====================================================================
% xxx.m - Test script for fsaverage6 Execution space
% Process all subjects dynamically, applies precision-weighting across runs, 
% computes contrast t-stats, and masks to BA9/46/7/40 (PFC, SPL, IPL).
% =====================================================================
...
for iSub = 1:length(subjects)
    ...
    for iSes = 1:length(sessions)
        ...
        % DataLog setup
        executionRuns = 1:3;
        nRuns = length(executionRuns);
        subjectLog = repmat({subID}, nRuns, 1);
        sessionLog = repmat({ses}, nRuns, 1);
        taskList = repmat({'Execution'}, nRuns, 1);
        runNum = executionRuns'; 
        ...
        % --- 7. Save outputs ---
        resultsDir = fullfile(bidsDir, 'derivatives', 'Execution6_BA9-46-7-40', subID, ses);
