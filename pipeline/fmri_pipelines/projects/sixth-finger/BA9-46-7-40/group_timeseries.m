...
% Two separate figures: BA46 and BA40 (native space, Glasser2016 labels).
% Data loaded from: derivatives/Imagery_native_BA9-46-7-40
%
% Lines plotted per figure:
...
imgBaseDir = fullfile(bidsDir, 'derivatives', 'Imagery_native_BA9-46-7-40');
fsSubBase  = fullfile(bidsDir, 'derivatives', 'freesurfer');

% --- ROI definitions ---
roiDefs(1).name   = 'BA46';
roiDefs(1).labels = {'46', '9-46d'};

roiDefs(2).name   = 'BA40';
roiDefs(2).labels = {'PF', 'PFcm', 'PFm', 'PFop', 'PFt'};
...
for iROI = 1:numel(roiDefs)
    ...
    for iSub = 1:nSubjects
        ...
        for iSes = 1:nSessions
            ...
            lhBet = fullfile(imgDir, ses, ['lh.' finger '.mgz']);
            rhBet = fullfile(imgDir, ses, ['rh.' finger '.mgz']);
