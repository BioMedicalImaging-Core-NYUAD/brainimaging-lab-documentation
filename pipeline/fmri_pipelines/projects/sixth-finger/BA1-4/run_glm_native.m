...
% (Execution + Imagery). Uses Glasser2016 labels for M1/S1 masking.
% =====================================================================
...
for iSub = 7:7%numel(subjects)
    ...
    % --- Load native-space M1/S1 mask (Glasser2016) ---
    ...
    fprintf('  Mask: %d vertices in M1/S1\n', sum(all_mask));
    ...
    for iSes = 1:numel(sessions)
        ...
        for iTask = 1:numel(taskDefs)
            ...
            for iFing = 1:nFingers
                ...
                MRIwrite(mgz_L, fullfile(resultsDir, sprintf('lh.%s_%d.mgz', fn, sesIdx)));
                mgz_R.vol = beta_combined(rightidx, iFing);
                MRIwrite(mgz_R, fullfile(resultsDir, sprintf('rh.%s_%d.mgz', fn, sesIdx)));
                mgz_L.vol = t_stat_combined(leftidx, iFing);
                MRIwrite(mgz_L, fullfile(resultsDir, sprintf('lh.%s_tstat_%d.mgz', fn, sesIdx)));
                mgz_R.vol = t_stat_combined(rightidx, iFing);
                MRIwrite(mgz_R, fullfile(resultsDir, sprintf('rh.%s_tstat_%d.mgz', fn, sesIdx)));
            end
            ...
            MRIwrite(mgz_L, fullfile(resultsDir, sprintf('lh.fingermap_%d.mgz', sesIdx)));
            mgz_R.vol = rh_fingmap;
            MRIwrite(mgz_R, fullfile(resultsDir, sprintf('rh.fingermap_%d.mgz', sesIdx)));
