...
% --- Configuration ---
taskType   = 'Imagery';  % 'Execution' or 'Imagery'
tThreshold = 1.645;      % Individual-level selectivity threshold (p < 0.10, two-tailed)

% Task-specific settings
if strcmp(taskType, 'Execution')
    inputFolder  = 'Execution6';   % read per-subject betas from here
    groupSuffix  = 'fsaverage6';   % group output subfolder within inputFolder
    fingerNames  = {'thumb', 'index', 'middle', 'ring', 'pinky'};
    fingerLabels = 1:5;
elseif strcmp(taskType, 'Imagery')
    inputFolder  = 'Imagery6';     % read per-subject betas from here
    groupSuffix  = 'fsaverage6_p10'; % group output subfolder within inputFolder
...
resultsBaseDir = fullfile(bidsDir, 'derivatives', inputFolder);
...
for iSub = 1:length(subjects)
    ...
    for iSes = 1:length(sessions)
        ses = sessions{iSes};
        sesN = str2double(regexp(ses, '\d+', 'match', 'once'));
        ...
        for iHemi = 1:2
            ...
            for iFing = 1:nFingers
                fpath = fullfile(subDir, sprintf('%s.%s_tstat_%d.mgz', hemi, fingerNames{iFing}, sesN));
                if ~exist(fpath, 'file')
                    allExist = false;
                    break;
                end
            ...
            mgz.vol = fingmap;
            MRIwrite(mgz, fullfile(subDir, sprintf('%s.fingermap_%d.mgz', hemi, sesN)));
        end
...
for iSes = 1:length(sesList)
    ...
    outDir = fullfile(resultsBaseDir, groupSuffix, ses);
    if ~exist(outDir, 'dir')
        mkdir(outDir);
    end
    ...
    for iHemi = 1:2
        ...
        for iSub = 1:length(subjects)
            ...
            for iFing = 1:nFingers
                tpath = fullfile(subDir, sprintf('%s.%s_tstat_%d.mgz', hemi, fingerNames{iFing}, sesN));
                bpath = fullfile(subDir, sprintf('%s.%s_%d.mgz', hemi, fingerNames{iFing}, sesN));
        ...
        for iFing = 1:nFingers
            ...
            MRIwrite(mgz, fullfile(outDir, sprintf('%s.%s_%d.mgz', hemi, fingerName, sesN)));

            % Save averaged t-stat
            mgz.vol = meanTstat(:, iFing);
            MRIwrite(mgz, fullfile(outDir, sprintf('%s.%s_tstat_%d.mgz', hemi, fingerName, sesN)));
        end

        % Save the subject count map (useful for QC)
        mgz.vol = countMap;
        MRIwrite(mgz, fullfile(outDir, sprintf('%s.nsubjects_%d.mgz', hemi, sesN)));
        ...
        mgz.vol = fingmap;
        MRIwrite(mgz, fullfile(outDir, sprintf('%s.fingermap_%d.mgz', hemi, sesN)));
